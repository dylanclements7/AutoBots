...
all_games = games_collection.find({})

# Build the games structure to match the expected format
games = {
    "games": [],
    "gameData": {}
}
...
for game in all_games:
    ...
    # Add to games list
    games["games"].append(game_title)
    
    # Add to gameData dict
    games["gameData"][game_title] = {
        "title": game_title,
        "html": game.get("html", ""),
        "description": game.get("game_summary", "")
    }

# Also expose as separate variables for backwards compatibility
...
async def get_games():
    ...
    all_games = list(games_collection.find({}, {"_id": 0}))
    
    # Return in the format expected by frontend
    return {
        "games": all_games,  # Full list of game objects
        "gameData": {game["title"]: game for game in all_games}  # Dict keyed by title
    }
...
async def get_lobby_status(game_type: str):
    ...
    """
    # Check if game exists using 'title' field, not 'name'
    game_exists = games_collection.find_one({"title": game_type})
    if not game_exists:
